...
const jwksUrl = 'https://dev-jw9s3mm5.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  let signedKey = await getSigningKey(jwksUrl, jwt.header.kid)
  return verify(token, signedKey.publicKey, { algorithms: ['RS256'] }) as JwtPayload
}
